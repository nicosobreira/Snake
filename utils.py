...
def addstr(scr, x: int, y: int, string: str, color=0):
    scr.addstr(y, x, string, color_pair(color))

def drawRectFrame(scr, x: int, y: int, sx: int, sy: int, strings: dict, color=0):
...
def drawRect(scr, x: int, y: int, sx: int, sy: int, string: str, color=0):
